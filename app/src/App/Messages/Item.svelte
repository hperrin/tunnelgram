      ...
      >
        {#if shouldEmbiggen}
          <div class="d-inline-block" on:click={() => flipFirst = !!flipper} style="font-size: 1.1rem">
            {#if message.decrypted.text != null}
              <span class="h1">{flipped ? message.decrypted.secretText : message.decrypted.text}</span>
        ...
        {:else}
          ...
          {#if flipped ? (formattedSecretText != null) : (formattedText != null)}
            <div class="card-body py-1 px-2 m-0" on:click={() => flipFirst = !!flipper}>
              <div class="message card-text markdown-body">{@html flipped ? formattedSecretText : formattedText}</div>
            </div>
          {/if}
        {/if}
      </div>
