<form on:submit|preventDefault={newMessage}>
    ...
    <div class="d-flex flex-wrap justify-content-start align-items-start position-relative bg-secondary pt-2 pl-2">
      {#each images as image, i}
        <div class="d-flex justify-content-center align-items-center mr-2 mb-2 border border-light position-relative" style="width: 152px; height: 152px;">
          ...
          <span class="badge badge-light thumbnailOverlay" style="left: 4px; bottom: 4px; pointer-events: none;">
            {image.dataWidth}x{image.dataHeight}
          </span>
          <span class="badge badge-light thumbnailOverlay" style="right: 4px; bottom: 4px; pointer-events: none;">
      ...
      {#if video != null}
        <div class="d-flex justify-content-center align-items-center mr-auto ml-auto mb-2 border border-light position-relative" style="min-width: 152px; min-height: 152px;">
          ...
          <button type="button" class="btn btn-sm btn-danger rounded-circle position-absolute" style="opacity: .8; right: 4px; top: 4px;" on:click={() => video = null}>
            <i class="fas fa-times text-white d-inline-block" style="width: 1em; height: 1em;"></i>
          </button>
      ...
      {#if mediaLoading === 'image'}
        <div class="d-flex justify-content-center align-items-center mr-2 mb-2 text-white" style="width: 152px; height: 152px;">
          <LoadingIndicator width="80" height="80" text="Resampling..." />
        </div>
      {/if}
      {#if mediaLoading === 'video'}
        <div class="d-flex justify-content-center align-items-center mr-auto ml-auto mb-2 text-white" style="width: 152px; height: 152px;">
          <LoadingIndicator width="150" height="150" text={transcoder == null ? 'Loading...' : (transcodeProgress == null ? 'Preparing...' : 'Transcoding...'+'\n'+Math.floor(transcodeProgress * 100)+'%')} progress={transcodeProgress} />
        </div>
        {#if transcoder != null}
          <button type="button" class="btn btn-sm btn-danger position-absolute" style="opacity: .8; right: 4px; top: 4px;" on:click={cancelVideoTranscode}>
            Cancel
          </button>
        {/if}
      {/if}
  ...
  <div class="d-flex align-items-stretch border-dark border-top">
    <input class="d-none" type="file" bind:this={fileInput} on:change={event => handleFiles(event.target.files)} accept="image/*, video/*" multiple>
    {#if $inCordova}
      <div class="add-media-dropdown btn-group dropup p-0" bind:this={addMediaDropdown}>
        <button class="btn btn-lg btn-success p-0 d-inline-flex justify-content-center align-items-center dropdown-toggle" style="width: 60px; min-width: 60px; font-size: .8em;" type="button" title="Add picture/video" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <i class="fas fa-camera"></i>&nbsp;/&nbsp;<i class="fas fa-video"></i>
        </button>
        <div class="dropdown-menu">
          <button class="dropdown-item p-3" type="button" on:click={cordovaCapturePhoto}>Take a photo</button>
          <button class="dropdown-item p-3" type="button" on:click={cordovaCaptureVideo}>Take a video</button>
          <div class="dropdown-divider"></div>
          <button class="dropdown-item p-3" type="button" on:click={() => fileInput.click()}>Browse library</button>
        </div>
      </div>
    {:else}
      <button class="btn btn-lg btn-success p-0 d-inline-flex justify-content-center align-items-center" style="width: 60px; min-width: 60px; font-size: .8em;" type="button" on:click={() => fileInput.click()} title="Add picture/video">
        <i class="fas fa-camera"></i>&nbsp;/&nbsp;<i class="fas fa-video"></i>
      </button>
    {/if}
    <textarea class="text-editor form-control border-0 text-dark" bind:value={text} placeholder="New message" on:keydown={event => handleKeyDown(event)} on:keyup={handleKeyUp} bind:this={textEditor}></textarea>
    <button class="btn btn-lg btn-primary" style="width: 60px; min-width: 60px;" type="submit" disabled={messageIsEmpty || mediaLoading} title="Send message">
...
<script>
  ...
  const round = (value, precision) => Number.parseFloat(value.toFixed(precision));
  ...
  $: messageIsEmpty = (text === '' || text.match(/^\s+$/)) && !images.length && video == null;
  ...
  function newMessage () {
    ...
    message.retrySave = () => message.save(true).then(removePending, ErrHandler);
  ...
  async function rotateImage (index) {
    ...
    let rotateThumbnail = new EditImageService(imageThumbnail, images[index].thumbnailType);
    ...
    const thumbnailMatch = thumbnailImg.match(/^data:(image\/\w+);base64,(.*)$/);
      ...
      err => {
        ...
        switch (err.code) {
          case CaptureError.CAPTURE_INTERNAL_ERR:
            errObj.textStatus = 'The camera or microphone failed to capture image or sound.';
            break;
          case CaptureError.CAPTURE_APPLICATION_BUSY:
            errObj.textStatus = 'The camera or audio capture application is currently serving another capture request.';
            break;
          case CaptureError.CAPTURE_INVALID_ARGUMENT:
            errObj.textStatus = 'Invalid use of the API (e.g., the value of limit is less than one).';
            break;
          ...
          case CaptureError.CAPTURE_NOT_SUPPORTED:
            errObj.textStatus = 'The requested capture operation is not supported.';
            break;
        }
        ErrHandler(errObj);
      },
  ...
  export async function handleFiles (files) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (file.type.startsWith('video/')) {
        ...
        if (!videoElem.videoWidth || !videoElem.videoHeight || !videoElem.duration) {
          PNotify.notice({
            title: 'Unreadable',
        ...
        if (file.type === 'video/mp4' && file.size < 20971520) {
          ...
          PNotify.info({
            title: 'Transcoding',
            text: file.size < 20971520
              ? 'Your video is formatted as '+file.type+'. It needs to be transcoded to video/mp4 to work on all devices. This may take a while.'
          ...
          try {
            ...
            data = await transcoder.transcode(new Uint8Array(result), videoElem.duration, progress => transcodeProgress = progress);
          } catch (err) {
            PNotify.error({
        ...
        PNotify.notice({
          ...
          text: 'Right now, '+$brand+' only supports images and videos. You tried to add: '+file.type
        });
