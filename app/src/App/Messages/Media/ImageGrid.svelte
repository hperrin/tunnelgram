    ...
    for (let i = 0; i < resources.length; i++) {
      if (!resources[i].thumbnailSrc && !resources[i].thumbnailSrcPromise) {
        if (resources[i].thumbnail instanceof Uint8Array) {
          const blob = new Blob([resources[i].thumbnail], {type: resources[i].thumbnailType});
          const _source = URL.createObjectURL(blob);
          resources[i].thumbnailSrc = _source;
        } else {
          resources[i].thumbnailSrcPromise = resources[i].thumbnail.then(data => {
            const blob = new Blob([data], {type: resources[i].thumbnailType});
            const _source = URL.createObjectURL(blob);
            resources[i].thumbnailSrc = _source;
          });
        }
...
<div bind:this={thumbnailContainer} class="d-flex flex-wrap justify-content-center bg-secondary text-white" style="max-width: {thumbnailDimension * thumbnailContainerMaxWidthMultiplier}px; font-size: 1em;">
