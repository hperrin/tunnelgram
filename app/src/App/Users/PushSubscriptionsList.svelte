      ...
      {#each subscriptions as subscription (subscription.guid)}
        <div class="list-group-item flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between align-items-start">
            <h5 class="mb-1">
              {#if showBrowser}
                {subscription.agent.browser.name || ''} {subscription.agent.browser.version || ''}
              {:else}
          ...
          <p class="mb-1 d-flex flex-column">
            {#if showBrowser}
              <span>{subscription.agent.os.name || ''} {subscription.agent.os.version || ''}</span>
            {/if}
            <span>{subscription.agent.device.vendor || ''} {subscription.agent.device.model || ''}</span>
          </p>
          <small title={new SimpleDateFormatter(subscription.cdate).format('wymdhms', 'short')}>Created: {new SimpleDateFormatter(subscription.cdate).format('ago', 'long')}</small>,
          <small title={new SimpleDateFormatter(subscription.mdate).format('wymdhms', 'short')}>Last used: {new SimpleDateFormatter(subscription.mdate).format('ago', 'long')}</small>
        </div>
      {/each}
    </div>
  {/if}
...
<script>
  ...
  let subscriptionsPromise = Nymph.getEntities({
    ...
    'type': '&',
    'ref': ['user', $user.guid]
  }).then(subs => subscriptions = subs.forEach(s => s.agent = UAParser(s.data.uaString)) || subs).catch(ErrHandler);
