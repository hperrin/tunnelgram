<div class="h-100 w-100" style="overflow-y: auto; -webkit-overflow-scrolling: touch; overscroll-behavior: contain;">
  <div class="d-flex flex-column align-items-center p-3">
    <div class="d-flex justify-content-center flex-wrap w-100">
      <div class="d-inline-block position-relative m-3" on:mouseenter={() => mouseOverAvatar = true} on:mouseleave={() => mouseOverAvatar = false}>
        {#if $viewUserIsSelf}
      ...
      {:else}
        <div>
          {#if existingConversations}
            ...
            {/if}
            <button type="button" class="btn {existingConversations.length ? 'btn-light' : 'btn-primary'} mt-3 w-100" on:click={newConversation} disabled={startingConversation}>Start a Chat with {$viewUser.data.nameFirst}</button>
          {:else if existingConversationsError}
            <div class="alert alert-danger my-3" role="alert">
              Oops... something went wrong.
            </div>
            <button type="button" class="btn btn-light mt-3 w-100" on:click={newConversation} disabled={startingConversation}>Start a Chat with {$viewUser.data.nameFirst}</button>
          {:else}
            <div>
              One second...
            </div>
    ...
    (async () => {
      try {
        const conversations = await Nymph.getEntities({
          ...
          'type': '&',
          'ref': [
            ['acFull', [$viewUser.guid, $user.guid]]
          ]
        });
